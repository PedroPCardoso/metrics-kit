...
- ~~Não suporta Prisma/Knex/Drizzle na v1.~~ **(Decisão 1, atualizada):** a abstração foi **extraída** quando o 2º backend surgiu (modo executor, ver §6.4). O core agora é dual-mode — o caminho TypeORM permanece intacto; um `ExecutorBackend` agnóstico de ORM emite SQL cru para qualquer `DataSource` `(sql, params) => rows`. Os adapters Prisma/Drizzle são publicados como subpaths do mesmo pacote (`nestjs-metrics/prisma`, `nestjs-metrics/drizzle`) — PRD #16.
